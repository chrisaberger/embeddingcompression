...
class LloydMaxQuantizer(Quantizer):
    ...
    def quantize(self, X):
        ...
        points = []
        for i in range(0,self.q_dim_row,num_rows_in_buck):
            for j in range(0,self.q_dim_col,num_cols_in_buck):
                i_prime = i + self.q_dim_row - 1
                j_prime = j + self.q_dim_col - 1
        ...
        entry_bytes = (
            np.ceil(np.log2(self.num_centroids)) * compressed_X.size/quant_dim) / 8
